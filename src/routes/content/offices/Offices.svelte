<script lang="ts">
	...
	import TransformDebug from './TransformDebug.svelte';

	import { timings } from './timings.js';
	import type { Component } from 'svelte';

	import data from './Hrr98Bdry_AK_HI_unmodified.geojson';
...
<Preview data={countries}>
	<div class="relative grid h-[600px] grid-cols-[224px,1fr]">
		<div class="absolute right-0 top-0 z-10 flex items-center gap-3">
			{#if isPlaying && selectedFeature}
				<span class="rounded-full bg-primary/5 px-2 py-1 text-sm font-semibold text-primary">
			...
			<svelte:component this={Context} {debug}>
				...
				{#each countries.features as country}
					<GeoPath
						geojson={country}
						class={cls(
							'stroke-surface-content/50 cursor-pointer fill-white',
